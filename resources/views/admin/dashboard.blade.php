...
<section class="admin-shell" style="background:#151210; min-height:100vh; display:flex;">
    ...
    <main class="flex-1 overflow-auto">
        <header class="px-6 lg:px-8 py-4 border-b flex items-center justify-between" style="background:var(--sidebar-bg); border-color:var(--border-dark);">
            <div>
                <h1 class="font-display font-medium text-xl" style="color:#f5e8e2;">Dashboard</h1>
        ...
        <div class="px-6 lg:px-8 py-8">
            ...
            <div class="admin-card rounded-2xl overflow-hidden">
                <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color:var(--border-dark);">
                    <div>
                        <h3 class="text-sm font-medium" style="color:#e8ddd8;">Recent Bookings</h3>
                        <p class="text-xs mt-0.5" style="color:#5a4e4a;">Latest 5 appointments</p>
                    </div>
                    <a href="{{ route('admin.bookings') }}" class="text-xs px-3 py-1.5 rounded-lg hover:opacity-80 transition-opacity" style="background:rgba(201,169,110,0.12); color:var(--gold);">
                        View All
                    </a>
                </div>
                ...
                @if($bookings->isEmpty())
                    <div class="py-16 text-center">
                        ...
                        <p class="text-xs" style="color:#5a4e4a;">Bookings will appear here once clients start booking.</p>
                    </div>
                @else
                            ...
                            <tbody>
                                @foreach($bookings->take(5) as $booking)
                                    <tr class="admin-row" style="border-bottom:1px solid var(--border-dark);">
                                        <td class="px-6 py-4 text-xs font-medium" style="color:#e8ddd8;">{{ $booking->full_name }}</td>
                                        ...
                                        <td class="px-4 py-4">
                                            <span class="badge-pending text-xs font-medium px-2.5 py-1 rounded-full">{{ ucfirst($booking->status) }}</span>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
